...
# 		Usage
# chmod +x SCR_5004_$version_NUAC-Sauvegarde-Volumes-EBS.sh
# ./SCR_5004_$version_NUAC-Sauvegarde-Volumes-EBS.sh           
...
function create_tar () {
	if [ -d $backup_destination/`get_lvs_name $1` ]; then
		echo $dir_exists;
	else
		$MKDIR $backup_destination/`get_lvs_name $1`;
	fi
		cd $backup_destination/`get_lvs_name $1`
		$TAR --exclude={"lost+found","mysql/data","mysql/tmp"} -czf `get_lvs_name $1`_$dateFile.tar.gz -C $mount_point . 
		$MD5SUM $backup_destination/`get_lvs_name $1`/`get_lvs_name $1`_$dateFile.tar.gz > $backup_destination/`get_lvs_name $1`/`get_lvs_name $1`_$dateFile.checksum
	
	if [ -f $backup_destination/`get_lvs_name $1`_$dateFileBefore.tar ]; then
		# Rotation des anciens fichiers 
		$RM $backup_destination/`get_lvs_name $1`_$dateFileBefore.tar;
		$RM $backup_destination/`get_lvs_name $1`_$dateFileBefore.checksum;
	else 
		echo $nothing;
	fi
}
