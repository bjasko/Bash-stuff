...
function mysql_backup () {
	$MYSQLDUMP --all-databases -u $mysql_backup_user -p$mysql_backup_pass > $backup_destination/$node_name/$mysql_backup_name-$dateFile.sql;
...
function create_tar () {
	...
	for i in ${paths[*]}; do
		...
		if [ $enable_checksum -eq 1 ]; then
			$SHA1SUM $backup_destination/$node_name/$filename-$dateFile.tar.gz > $backup_destination/$node_name/$filename-$dateFile.checksum
		fi
		...
		backup_size=`$DU -h $backup_destination/$node_name/$filename-$dateFile.tar.gz | $CUT -f 1`
		echo -e "$backup_destination/$node_name/$filename-$dateFile.tar.gz - $hours h $minutes m and $seconds seconds. Size - $backup_size" >> $email_tmp_file
	done
}
